...
<div class="flex">
    ...
    <button @click="$store.sidebar.navOpen = !$store.sidebar.navOpen"
        ...
        {{-- Menu icon --}}
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" x-bind:class="$store.sidebar.navOpen ? 'hidden' : ''"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" />
        </svg>
        {{-- Close menu --}}
        <svg x-cloak xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
    ...
    {{-- Sidebar --}}
    <div class="bg-primary-500 fixed transition-all duration-300 space-y-2 sm:relative" x-bind:class="{ 'w-64': $store.sidebar.full, 'w-64 sm:w-20': !$store.sidebar.full, 'top-0 left-0': $store.sidebar
        ...
        <div class="h-14 bg-white border-b md:px-8 text-sm flex justify-between items-center">
            ...
            <div x-data="{ open: false }">
                <div class="flex items-center cursor-pointer select-none group" @click="open = true">
                    <div
                        class="text-gray-700 group-hover:text-primary-500 focus:text-primary-500 mr-4 whitespace-no-wrap">
                        <span class="hidden md:inline">{{ auth()->user()->nama }}</span>
                    </div>
                    <img class="w-6 h-6 rounded-full" src="{{ $photo ?? null }}">
                </div>
                <ul class="sm:mr-4 md:mr-12 lg:mr-12 text-right right-0 mt-2 py-2 shadow bg-white rounded text-sm absolute z-50"
                    x-show.transition.duration.50ms="open" @click.away="open = false">
