...
<div class="px-4 space-y-5">
    ...
    <div x-data="tooltip"
        ...
        x-bind:class="{'justify-start': $store.sidebar.full, 'sm:justify-center':!$store.sidebar.full,'text-gray-200 bg-primary-600':$store.sidebar.active == 'home','text-gray-400 ':$store.sidebar.active != 'home'}">
    ...
    <div x-data="dropdown"
            class="relative">
        <!-- Dropdown head -->
        <div @click="toggle('tindak-lanjut')"
            x-data="tooltip"
            x-on:mouseover="show = true"
            x-on:mouseleave="show = false"
            ...
            x-bind:class="{'justify-start': $store.sidebar.full, 'sm:justify-center':!$store.sidebar.full, 'text-gray-200 bg-gray-800':$store.sidebar.active == 'tindak-lanjut','text-gray-400 ':$store.sidebar.active != 'tindak-lanjut'}">
            <div class="relative flex space-x-2 items-center">
                @include('components.icon', ['name' => 'rectangle-stack', 'size' => 'w-5 h-5'])
                <h1 x-cloak x-bind:class="!$store.sidebar.full && show ? visibleClass :'' || !$store.sidebar.full && !show ? 'sm:hidden':''">Tindak Lanjut</h1>
            </div>
    ...
    <div x-data="dropdown"
        ...
        <div @click="toggle('laporan')"
            ...
            x-on:mouseleave="show = false"
            class="flex justify-between text-gray-400 hover:text-gray-200 hover:bg-gray-800 items-center space-x-2 rounded-md p-2 cursor-pointer"
            x-bind:class="{'justify-start': $store.sidebar.full, 'sm:justify-center':!$store.sidebar.full, 'text-gray-200 bg-gray-800':$store.sidebar.active == 'laporan','text-gray-400 ':$store.sidebar.active != 'laporan'}">
            <div class="relative flex space-x-2 items-center">
                @include('components.icon', ['name' => 'presentation-chart', 'size' => 'w-5 h-5'])
                <h1 x-cloak x-bind:class="!$store.sidebar.full && show ? visibleClass :'' || !$store.sidebar.full && !show ? 'sm:hidden':''">Laporan</h1>
            </div>
    ...
    <div x-data="dropdown"
            class="relative">
        <!-- Dropdown head -->
        <div @click="toggle('pengaturan')" x-data="tooltip" x-on:mouseover="show = true" x-on:mouseleave="show = false" class="flex justify-between text-gray-400 hover:text-gray-200 hover:bg-gray-800 items-center space-x-2 rounded-md p-2 cursor-pointer"
            x-bind:class="{'justify-start': $store.sidebar.full, 'sm:justify-center':!$store.sidebar.full, 'text-gray-200 bg-gray-800':$store.sidebar.active == 'pengaturan','text-gray-400 ':$store.sidebar.active != 'pengaturan'}">
            <div class="relative flex space-x-2 items-center">
                @include('components.icon', ['name' => 'cog', 'size' => 'w-5 h-5'])
                <h1 x-cloak x-bind:class="!$store.sidebar.full && show ? visibleClass :'' || !$store.sidebar.full && !show ? 'sm:hidden':''">Pengaturan</h1>
            </div>
            <svg x-cloak x-bind:class="$store.sidebar.full ? '':'sm:hidden'" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
            </svg>
        ...
        <!-- Dropdown content -->
        <div x-cloak x-show="open" @click.outside="open = false" x-bind:class="$store.sidebar.full ? expandedClass : shrinkedClass" class="text-gray-400 space-y-3">
            <h1 class="hover:text-gray-200 cursor-pointer">
