                                    ...
                                    @foreach($users as $index => $user)
                                        <tr>
                                            <th scope="row" class="align-middle">
                                                <a class="text-dark" href="{{ route('users.edit', $user->id) }}">{{ $index + 1 }}</a>
                                            </th>
                                            <td class="align-middle">
                                                <a class="text-dark" href="{{ route('users.edit', $user->id) }}">{{ $user->nama }}</a>
                                            </td>
                                            <td class="align-middle">
                                                <a class="text-dark" href="{{ route('users.edit', $user->id) }}">{{ $user->email }}</a>
                                            </td>
                                            <td class="align-middle">
                                                @if(!is_null($user->bpsid))
                                                    <a class="text-dark" href="{{ route('users.edit', $user->id) }}">{{ $user->bpsid }}</a>
                                                @else
                                                    -
                                                @endif
                                            </td>
