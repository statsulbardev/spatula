        ...
        <ul class="flex flex-nowrap" role="tablist" data-te-nav-ref>
            <li class="border-l-2 border-t-2 border-b-2 border-primary-100 drop-shadow-sm rounded-l-md bg-white px-4 py-2 leading-tight hover:bg-gray-300" role="presentation">
                <a
                    ...
                    class="block text-sm text-neutral-500 hover:isolate focus:isolate data-[te-nav-active]:text-primary-500 data-[te-nav-active]:font-bold"
                    data-te-toggle="pill"
    ...
    <section class="mb-6 mt-10">
        <div class="w-full overflow-x-auto rounded bg-white shadow">
            <div class="px-4 pt-2 pb-4">
                ...
                <select id="tahun" wire:model.live="selectedYear" class="form-select max-w-48">
            ...
            <div class="mb-6">
                ...
                <div class="hidden opacity-100 transition-opacity duration-150 ease-linear data-[te-tab-active]:block"
                    ...
                    <table wire:key="{{ Str::random() }}" class="w-full table-auto">
                        <thead>
                            <tr class="bg-neutral-100 text-left font-bold">
                                @foreach ($officerRating[0] as $columnOfficer)
                                    <th class="px-6 pb-4 pt-6 border border-white">{{ $columnOfficer }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($officerRating[1] as $monthIndex => $reportOfficer)
                                <tr>
                                    ...
                                    @else
                                        <td class="border" rowspan="{{ $reportOfficer->count() + 1 }}">
                                            <span class="items-center py-4 pl-6">
                                                {{ array_column($this->months, $monthIndex)[0] }}
                                            </span>
                                        </td>
                    ...
                    <table wire:key="{{ Str::random() }}" class="w-full table-auto">
                        ...
                        <tbody>
                            ...
                            <tr class="focus-within:bg-grey-lightest">
                                ...
                                @else
                                <td class="border" rowspan="{{ $reportService->count() + 1 }}">
                                    <span class="items-center py-4 pl-6">
                                        {{ array_column($this->months, $monthIndex)[0] }}
                                    </span>
                                </td>
                ...
                {{-- Saran Pengaduan --}}
                <div class="hidden opacity-0 transition-opacity duration-150 ease-linear data-[te-tab-active]:block"
                    id="saran-pengaduan" role="tabpanel" aria-labelledby="saran-pengaduan-tab">
                    <table wire:key="{{ Str::random() }}" class="w-full table-auto">
                        <thead>
                            <tr class="bg-neutral-100 text-left font-bold">
                        ...
                        <tbody>
                            ...
                            <tr class="focus-within:bg-grey-lightest">
                                <td class="border" rowspan="{{ $report->count() + 1 }}">
                                    <span class="items-center py-4 pl-6">
                                        {{ array_column($this->months, $monthIndex)[0] }}
                                    </span>
                                </td>
