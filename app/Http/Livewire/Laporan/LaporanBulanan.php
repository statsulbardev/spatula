...
class LaporanBulanan extends Component
{
    ...
    public function mount()
    {
        ...
        $this->monthlyReport = $this->getOfficerRating($this->selectedYear, Auth::user()->hasRole('superadmin'));
    }
    ...
    private function getOfficerRating($year, $role)
    {
        if ($role) {
            $result = DB::select('SELECT MONTH(a.created_at) as bulan, b.nama , AVG(rating_petugas) as rerata, COUNT(rating_petugas) as jumlah_terlayani
                    FROM d_penilaian a, m_pengguna b WHERE a.kode_petugas=b.id and YEAR(a.created_at) = ' . $year . ' GROUP BY MONTH(a.created_at), kode_petugas');
        } else {
            $result = DB::select('SELECT MONTH(a.created_at) as bulan, b.nama , AVG(rating_petugas) as rerata, COUNT(rating_petugas) as jumlah_terlayani
                    FROM d_penilaian a, m_pengguna b
                    WHERE a.kode_satker_id = ' . $this->getUnitCode()->kode_satker . ' AND a.kode_petugas=b.id AND YEAR(a.created_at) = '. $this->selectedYear .'
    ...
    private function getServiceRating($role)
    {
        if ($role === 'superadmin') {
            $result = DB::select('SELECT MONTH(a.created_at) as bulan, b.nama_layanan , AVG(rating_layanan) as rerata, COUNT(rating_layanan) as jumlah_terlayani
                    FROM d_penilaian a, m_layanan b
                    WHERE a.kode_layanan=b.kode_layanan AND YEAR(a.created_at) = '. $this->selectedYear .' GROUP BY MONTH(a.created_at), a.kode_layanan');
        } else {
            $result = DB::select('SELECT MONTH(a.created_at) as bulan, b.nama_layanan , AVG(rating_layanan) as rerata, COUNT(rating_layanan) as jumlah_terlayani
    ...
    private function getComplaintSuggestion($role)
    {
        if ($role === 'superadmin') {
            $result = DB::select("SELECT MONTH(a.created_at) as bulan, b.nama_saran , COUNT(a.kode_saran) as jumlah_saran
                    FROM d_penilaian a, m_saran b
                    WHERE YEAR(a.created_at) = '. $this->selectedYear .' AND a.kode_saran LIKE CONCAT('%', b.kode_saran ,'%')
