        ...
        if ($role) {
            ...
            $result = DB::select(
                'SELECT MONTH(a.created_at) as bulan, b.nama_layanan , AVG(rating_layanan) as rerata, COUNT(rating_layanan) as jumlah_terlayani
                    FROM d_penilaian a, m_layanan b WHERE a.kode_satker_id = ' .
                    $this->getUnitCode()->kode_satker .
                    " AND a.kode_layanan=b.kode_layanan AND YEAR(a.created_at) = " .
                    $this->selectedYear .
                    '
    ...
    private function getComplaintSuggestion($role)
    {
        ...
        if ($role) {
            ...
            foreach ($result as $index => $d) {
                $data[$index] = $d->flatten()->countBy();
            }
        } else {
            $result = DB::select(
                "SELECT MONTH(a.created_at) as bulan, b.nama_saran , COUNT(a.kode_saran) as jumlah_saran
                    FROM d_penilaian a, m_saran b WHERE a.kode_satker_id = " .
                    $this->getUnitCode()->kode_satker .
                    " AND YEAR(a.created_at) = '. $this->selectedYear .' AND a.kode_saran LIKE CONCAT('%', b.kode_saran ,'%')
